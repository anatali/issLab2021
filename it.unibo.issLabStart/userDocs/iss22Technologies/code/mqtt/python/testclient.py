...
class Client(object):
    ...
    * Use disconnect() to disconnect from the broker
    ...
    def connect_async(self, host, port=1883, keepalive=60, bind_address=""):
        ...
        #print("here state",self._state) #steve

    def reconnect(self):
        #print("in reconnect",) #steve
    ...
    def _message_retry_check_actual(self, messages, mutex):
        ...
        now = time_func()
        for m in messages:
            #print("outgoing messages queued ",len(messages))
    ...
    def _handle_publish(self):
        ...
        if sys.version_info[0] >= 3:
            message.topic = message.topic.decode("utf-8","ignore") #steve
            #message.topic = message.topic.decode("utf-8") #steve
    ...
    def _handle_pubrec(self):
        ...
        mid = struct.unpack("!H", self._in_packet['packet'])
        mid = mid[0]
        if not self.suppress_pubrel_flag: #steve
    ...
    def _do_on_publish(self, idx, mid):
        ...
        #print("messages in out queue = ",len(self._out_messages))#steve
        return MQTT_ERR_SUCCESS
    ...
    def _handle_pubackcomp(self, cmd):
        ...
        if self.suppress_puback_flag:#steve:
            self._easy_log(MQTT_LOG_DEBUG, "Received "+cmd+" Mid: "+str(mid)+" Discarded")
        else:
            self._easy_log(MQTT_LOG_DEBUG, "Received "+cmd+" Mid: "+str(mid))
        self._out_message_mutex.acquire()
        for i in range(len(self._out_messages)):
            try:
                ...
                if self._out_messages[i].mid == mid:
                    ...
                    if not self.suppress_puback_flag:#steve
                        if cmd=="PUBCOMP":
                                print("PuBCOMP Not suppressed")#steve
                        else:
                                print("PuBack Not suppressed")#steve
