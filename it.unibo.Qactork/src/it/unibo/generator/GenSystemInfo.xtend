...
class GenSystemInfo {
	 def void  doGenerate( QActorSystemSpec system,  boolean tracing,  boolean msglogging, SysKb kb ){
	 	GenUtils.genFileDir(   "..",  "", system.name, "pl", genCtx( system, tracing, msglogging  ) )	
		GenUtils.genFileDir(  "..",  "",  "sysRules" , "pl", genCtxRules() )
	}

/*
 * SYSTEM DESCRIPTION FILE CONTENT
 */	
def genCtx(QActorSystemSpec system,  boolean tracing, boolean msglog )
'''
%====================================================================================
% «system.name» description   
%====================================================================================
«IF system.mqttBroker !==  null »mqttBroker("«system.mqttBroker.brokerHost»", "«system.mqttBroker.brokerPort»", "«system.mqttBroker.eventtopic»").«ENDIF»
«FOR  ctx : system.context »«genContext(ctx)»
«ENDFOR»
«FOR  qa : system.actor » «genQactorDescr(qa)» «ENDFOR»
«IF tracing»tracing.
«ENDIF»
«IF msglog»msglogging.
«ENDIF»
'''

def genContext( Context ctx)'''
	«IF ctx.mqtt»context(«ctx.name.toLowerCase», "«ctx.ip.host»",  "MQTT", "«ctx.ip.port»").
	«ELSE»context(«ctx.name.toLowerCase», "«ctx.ip.host»",  "TCP", "«ctx.ip.port»").«ENDIF»
'''

def dispatch genQactorDescr(QActorDeclaration qa)	{}
def dispatch genQactorDescr(QActorExternal qa)
'''qactor( «qa.name.toLowerCase», «qa.context.name.toLowerCase», "external").
'''
def dispatch genQactorDescr(QActor qa)
'''qactor( «qa.name.toLowerCase», «qa.context.name.toLowerCase», "it.unibo.«qa.name».«qa.name.toFirstUpper»").
'''
def dispatch genQactorDescr(QActorCoded qa)
'''qactor( «qa.name.toLowerCase», «qa.context.name.toLowerCase», "«qa.className»").
'''
/*
