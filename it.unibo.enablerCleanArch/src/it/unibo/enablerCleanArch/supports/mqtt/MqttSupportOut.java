...
public static final String topicOut = "topicCtxMqtt";
	...
	protected void connect(String clientid, String topic, String brokerAddr) {
		...
		try {
			...
			isConnected = true;
			ColorsOut.out("MqttSupport | connected " + clientid + " to " + topic, ColorsOut.BgYellow);
	...
	public void connectMqtt(String clientid, String topic, IApplMsgHandler handler) {
		...
		ColorsOut.out(clientid + " | CREATED MqttSupport handler="+handler + " subscribed to " + topic);
	}
	...
	public void publish(String topic, String msg, int qos, boolean retain) {
		...
		try {
			ColorsOut.out("MqttSupport  | publish topic=" + topic + " msg=" + msg + " client=" + client);
			message.setPayload(msg.getBytes());		 
			client.publish(topic, message);
		} catch (MqttException e) {
			ColorsOut.outerr("MqttSupport  | publish Error " + nattempts + ":" + e.getMessage());
			if( nattempts++ > 3 ) { 
				ColorsOut.outerr("MqttSupport  | publish NO MORE POSSIBLE"  );
				System.exit(10);
...
//----------------------------------------------------	
	...
	public String request(String msg) throws Exception { //msg should contain the name of the sender
		...
		try{
			...
			ColorsOut.outerr("MqttSupport | request ERROR:" + e.getMessage());
		...
		while( answer== null ) {
			...
			ColorsOut.out("MqttSupport | blockingQueue poll answer=" + answer  );
			Utils.delay(500); //il client ApplMsgHandler dovrebbe andare ...
		}
		ColorsOut.out("MqttSupport | request answer=" + answer + " blockingQueue=" + blockingQueue);
 		try {
 			...
 			ColorsOut.out("MqttSupport | receiveMsg2 " + answer + " not structured"   ); 			
 		}
		return answer;
 	}
	...
	public void reply(String msg) throws Exception {
		try {
			...
			ColorsOut.out("MqttSupport | reply topic=" + topic  + " msg="+msg);
	...
	protected String receiveMsg(String topic, BlockingQueue<String> bq) throws Exception{
		ColorsOut.out("MqttSupport | receiveMsg2 topic=" + topic + " blockingQueue=" + bq);
  		String answer = bq.take();
		ColorsOut.out("MqttSupport | receiveMsg2 answer=" + answer + " blockingQueue=" + bq);
 		try {
	...
	protected String receiveMsg(String topic) throws Exception{
		ColorsOut.out("MqttSupport | receiveMsg topic=" + topic + " blockingQueue=" + blockingQueue);
		//subscribe(topic);
 		String answer = blockingQueue.take();
		ColorsOut.out("MqttSupport | receiveMsg answer=" + answer + " blockingQueue=" + blockingQueue);
 		try {
 			ApplMessage msg = new ApplMessage(answer); //answer is structured
 			answer = msg.msgContent(); 			
 		}catch(Exception e) {
 			ColorsOut.out("MqttSupport | receiveMsg " + answer + " not structured"   ); 			
 		}
		client.unsubscribe(topic);
		return answer;		 
	}
	...
	public String receiveMsg() throws Exception {		
		ColorsOut.out("MqttSupport | receiveMsg subscribes topic=" + topic + " blockingQueue=" + blockingQueue);
		//subscribe(topic);
 		String answer = blockingQueue.take();
		ColorsOut.out("MqttSupport | receiveMsg subscribes answer=" + answer + " blockingQueue=" + blockingQueue);
 		client.unsubscribe(topic);
		return answer;
	}
