...
public class HIController {
    ...
    private static final String robotName  = "vr";
    ...
    public String configure(Model viewmodel  , @RequestParam String move, String addr ){
        ColorsOut.outappl("doMove:" + move, ColorsOut.BLUE);
        //configure(addr);
    ...
    public String doMove(Model viewmodel  , @RequestParam String move ){
        ...
        ColorsOut.outappl("doMove:" + move + " conn=" + conn, ColorsOut.BLUE);
        try {
            conn.forward( moveAril(move).toString() );
        } catch (Exception e) {
            e.printStackTrace();
        }
        return "RobotNaiveGui";
